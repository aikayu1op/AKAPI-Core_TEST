...
import { IPlayerSneakingEvents } from "./interface.js";
  ...
  let runId = system.allPlayerTickSubscribe(({ player }) => {
    ...
    _event.forEach((f) => {
      if (player.isSneaking) {
        ...
        let sneakTime = f.sneakTime.has(player.id)
          ? f.sneakTime.get(player.id) >= Number.MAX_SAFE_INTEGER
            ? (f.sneakTime.set(player.id, 0), 0)
            : f.sneakTime.get(player.id)
          : 0;
