 ...
 */
/**
 * @type {Object.<string, {texture: string, type: 'block' | 'item'}>}
 */
export const custom_ui_content = {
	/*
	'custom:block': {
...
const custom_ui_content_keys = new Set(Object.keys(custom_ui_content));
...
class ChestFormData {
	...
	button(slot, itemName, itemDesc, texture, stackSize = 1, durability = 0, enchanted = false) {
		const targetTexture = custom_ui_content_keys.has(texture) ? custom_ui_content[texture]?.texture : texture;
	...
	pattern(pattern, key) {
		for (let i = 0; i < pattern.length; i++) {
			...
			for (let j = 0; j < row.length; j++) {
				...
				if (!data) continue;
				const slot = j + i * 9;
				const targetTexture = custom_ui_content_keys.has(data.texture) ? custom_ui_content[data.texture]?.texture : data.texture;
				const ID = typeIdToDataId.get(targetTexture) ?? typeIdToID.get(targetTexture);
	...
	show(player) {
		...
		if (!inventory_enabled) return form.show(player.getMCPlayer());
		/** @type {Container} */
		...
		for (let i = 0; i < container.size; i++) {
			...
			const targetTexture = custom_ui_content_keys.has(typeId) ? custom_ui_content[typeId]?.texture : typeId;
			const ID = typeIdToDataId.get(targetTexture) ?? typeIdToID.get(targetTexture);
		...
		return form.show(player.getMCPlayer());
	}
}
